...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            url = self.base_link + self.search_link % (query.replace('-', '%20'))
            ...
            if 'tvshowtitle' in data:
                ...
                data = ensure_text(self.session.get(url, headers=self.session.headers).content, errors='replace')
                link = client.parseDOM(data, 'a', ret='href')
                url = [i for i in link if sepi in i][0]
            r = ensure_text(self.session.get(url, headers=self.session.headers).content, errors='replace')
            try:
